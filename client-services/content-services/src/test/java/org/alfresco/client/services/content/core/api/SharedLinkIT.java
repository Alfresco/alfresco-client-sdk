...
public class SharedLinkIT extends CSAPITestCase
{
    ...
    @Test(groups = { "with-solr", "core" })
    public void retrieveSharedLinks() throws IOException
    {
        // Retrieve Info from discovery
        RepositoryInfoRepresentation info = client.getDiscoveryAPI().getRepositoryInformationCall().execute().body();
        if (!info.getStatus().isQuickShareEnabled()) { return; }
