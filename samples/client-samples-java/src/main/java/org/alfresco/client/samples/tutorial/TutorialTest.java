...
public class TutorialTest {
    ...
    static void tutorialInitiation() throws IOException {
        ...
        client = new CSClient.Builder().httpLogging(HttpLoggingInterceptor.Level.BODY)
    ...
    static void tutorialPart4ManagingNodes() throws IOException {
        ...
        FileRepresentation emptyNode = (FileRepresentation) emptyNodeResponse.body();
        Assert.assertEquals(emptyNode.getContent().getSizeInBytes(), 0);
        ...
        Assert.assertEquals(((FileRepresentation) updatedContentNode).getContent().getSizeInBytes(), 41);
    ...
    static void tutorialPart5VersioningLocking() throws IOException {
        ...
        RequestBody requestBody = RequestBody.create(MediaType.parse("text/plain"), "This is the initial content for the file.");
        FileRepresentation updatedContentNode = (FileRepresentation) nodesAPI
                .updateNodeContentCall(nodeId, requestBody, true, "First version").execute().body();
        ...
        FileRepresentation minorupdatedContentNode = (FileRepresentation) nodesAPI
                .updateNodeContentCall(nodeId, minorRequestBody, false, "Second version").execute().body();
    ...
    static void tutorialPart8Sites() throws IOException{
        ...
        CSClient client2 = new CSClient.Builder().httpLogging(HttpLoggingInterceptor.Level.BODY)
       ...
       /* ResultPaging<SiteMemberRepresentation> siteMembers = client2.getSitesAPI().listSiteMembershipsCall("publicSite").execute().body();
        ...
        Assert.assertEquals(client2.getSitesAPI().listSiteMembershipsForPersonCall("test2").execute()
                .body().getCount(), 1);

        //Delete Site
        Response<Void> deleteSiteResponse = client.getSitesAPI().deleteSiteCall("publicSite")
                .execute();
    ...
    static void tutorialPart10People() throws IOException{
        ...
        //Find People
        ResultPaging<PersonRepresentation> searchPersonList = client.getQueriesAPI()
                .findPeopleCall("jdoe").execute().body();
    ...
    static void tutorialPart11Trashcan() throws IOException {
        ...
        RequestBody fileRequestBody = multipartBuilder.build();
        NodeRepresentation nodeToDelete = client.getNodesAPI()
                .createNodeCall(NodesAPI.FOLDER_MY, fileRequestBody).execute().body();

        //Delete file
        client.getNodesAPI().deleteNodeCall(nodeToDelete.getId()).execute();
        Assert.assertFalse(
                client.getNodesAPI().getNodeCall(nodeToDelete.getId()).execute().isSuccessful());
        ...
        Assert.assertEquals(deletedNodeInfo.getClass(), DeletedFileRepresentation.class);
